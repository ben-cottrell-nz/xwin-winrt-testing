...
void AppView::OnDisplayContentsInvalidated(const DisplayInformation& sender, const IInspectable& args)
{
    ...
    ChkOk(deviceAdapter->GetParent(IID_IDXGIFactory4, deviceFactory.put_void()));
    ...
    ChkOk(CreateDXGIFactory1(IID_IDXGIFactory4, currentFactory.put_void()));
...
void AppView::CreateWindowSizeDependentResources()
{
    ...
    else
    {
        ...
        ChkOk(dxgiAdapter->GetParent(IID_IDXGIFactory4, dxgiFactory.put_void()));
    ...
    D3D11_TEXTURE2D_DESC1 depthStencilDesc(
        ...
        static_cast<DXGI_SAMPLE_DESC>(D3D11_BIND_DEPTH_STENCIL));

    com_ptr<ID3D11Texture2D1> depthStencil;
    ChkOk(m_device->CreateTexture2D1(&depthStencilDesc, nullptr, depthStencil.put()));

    D3D11_DEPTH_STENCIL_VIEW_DESC depthStencilViewDesc{ .ViewDimension=D3D11_DSV_DIMENSION_TEXTURE2D };
    ChkOk(m_device->CreateDepthStencilView(depthStencil.get(), &depthStencilViewDesc, m_depthStencilView.put()));

    // Set the 3D rendering viewport to target the entire window.
    m_viewport = D3D11_VIEWPORT(0.0f, 0.0f, m_renderTargetSize.Width, m_renderTargetSize.Height);
    m_context->RSSetViewports(1, &m_viewport);
}
